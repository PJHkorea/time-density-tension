...
$$
H_0^{\text{TDT}} = \frac{c_{\mathrm{univ}}}{\alpha \cdot \ln 2} \cdot \left( \frac{\gamma}{\Omega_1} \right) \cdot \kappa_{\mathrm{conformal}} \approx 67.8055 \text{ km/s/Mpc}
$$
...
$$
H_0^{\text{local}}(a) = H_0^{\text{TDT}} \cdot \left[ 1.0 + \delta_{\mathrm{phase}} \cdot \cosh\left( \frac{\pi}{\sqrt{3}} \cdot a \right) \right]
$$


Under the frozen parameter layout ($\delta_{\mathrm{phase}} \equiv \alpha \approx 0.007297$), evaluating this formulation at the recombination horizon limit ($a \to 0$) and the contemporary local metric baseline ($a \to 1$) yields the following boundary conditions:
