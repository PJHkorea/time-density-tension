...
class BulletClusterTDTSimulator:    
    ...
    def run_collision_simulation(self, steps=500):
        ...
        def get_tension_acceleration(p, v):
            ...
            # 2. 원점 돌파 후 멀어질 때(abs(p) 기준 2D 동심원 확장 시), 고무줄 탄성 가동
            if abs(p) > 5.0:
                # 거리에 비례하여 장력이 증발하는 분모 댐핑을 정밀 상쇄 (지수 1.8 정합)
                conformal_pull_scaler = 1.0 + (r / self.grid_initial_slip_kpc) ** 1.8
                base_accel = base_accel * conformal_pull_scaler
                
                # 속도와 거리에 동시 비례하는 거시 복원 드래그 융합
                base_accel += 0.05 * (r / self.grid_initial_slip_kpc) * abs(v)
            
            # [교정 완료] 언제나 절대 좌표 원점(0.0)을 바라보고 당겨주는 기하학적 복원 부호로 변환
            # 이 수식을 통과하면 p < 0 일 때는 pull_direction = +1.0 이 되어 원점으로 가속하고,
            # 원점을 지나 p > 0 이 되면 pull_direction = -1.0 이 되어 전진을 멈추는 완벽한 고무줄 텐션이 가동됩니다.
