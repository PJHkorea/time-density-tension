...
def execute_tdt_simulation_part1(core: TDTCore):
    ...
    for r, v_baryon in zip(radii_sample, v_baryon_presets):
        # 1. Projects radius r onto the informational lattice effective wavenumber axis.
        # Couples the internal baryonic fluid radiation pressure resistance with the underlying spatial baseline deformation.
        effective_r_axis = (r - 1.0) * (1.0 - (core.delta_phase / np.sqrt(3.0))) if r > 1.0 else 0.0
        
        # 2. Implements the Tracy-Widom distribution manifold in the denominator to suppress divergent tension runaways at the galactic boundary.
        tracy_widom_galaxy = np.exp((core.gamma * effective_r_axis) ** 1.5)
        
        # 3. Computes the baseline galactic phase tension velocity derived from a pristine combination of first-principles constants.
        v_tension_bare = (core.c_univ * omega_1 * macro_scale_factor * (r ** core.gamma)) / tracy_widom_galaxy
        
        # Maps the coordinate boundary transformation to project v_tension into the standard astronomical km/s scale margin.
        v_tension = v_tension_bare * 0.045
        
        # 4. Synthesizes the continuous geometric baryon-tension composite velocity profile.
        v_total_bare = np.sqrt(v_baryon**2 + v_tension**2)
        
        # 5. Applies final fluid viscosity corrections driven by Debye damping shielding (Standardized stasis value R_d = 3.5 kpc).
        r_d = 3.5
