...
class TDTCore:
    ...
    def __init__(self, num_anchors: int = 30):
        # ---------------------------------------------------------------------
        # 1. 근본 물리 상수 및 위상학적 상수 선언 (닫힌 수식 불변 조건 유지)
        # ---------------------------------------------------------------------
        ...
        self.delta_phase = 0.039513       # 중입자 유체 위상 편이 디폴트 상수
        ...
        # [추가 리팩토링]: 타원/특이 은하의 비정상적 고밀도 가스 영역을 조절할 동적 점성 상수
        self.friction_decay_rate = 4.0    # 디폴트 유체 마찰 감쇄 반경 (kpc 스케일 척도)
    ...
    def calculate_dynamic_friction(self, radius: np.ndarray, baryon_mass: np.ndarray) -> np.ndarray:
        """
        [전격 추가 - 중입자 유체 동적 점성 필터]
        타원/특이 은하의 비정상적 고밀도 가스 중심부 마찰을 지능적으로 제어합니다.
        질량이 매우 무거운 은하계 내부일수록 유체 점성이 강해져 인장력을 조절하는 스위치 역할을 수행합니다.
        """
        radius_arr = np.atleast_1d(np.array(radius, dtype=float))
        mass_arr = np.atleast_1d(np.array(baryon_mass, dtype=float))
        ...
        # 은하 질량 체급에 따라 중심부 마찰 진폭(\delta_phase)을 동적으로 조절
        # 무거운 은하일수록 중심부 가스 저항 계수를 비선형적으로 제어 (\Delta 마찰 계수)
        dynamic_delta = self.delta_phase * (mass_ratio ** -0.05)
        
        # 가스 감쇄 반경(R_Debye) 역시 은하 체급에 비례하도록 동적 분모 보정
        dynamic_decay = self.friction_decay_rate * (mass_ratio ** 0.12)
        
        # 최종 동적 유체 마찰 감쇄 벡터 산출
        fluid_damping = dynamic_delta * np.exp(-radius_arr / dynamic_decay)
...
try:
    ...
    print("✅ [성공] SciPy 버전을 우회하여 TDT 코어 엔진, 가변 중력 및 동적 점성 스위치가 정상 등록되었습니다!")
...
try:
    ...
    data['v_tdt_predicted'] = v_total_bare * (1.0 + dynamic_fluid_friction)
