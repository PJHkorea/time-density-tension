...
def run_tdt_upsilon_validation(df_cleaned: pd.DataFrame):
    ...
    for gal in galaxies:
        ...
        # [동결 검증 모드] 대안 A 적용 목적 함수 (자유도 박탈 및 1개 파라미터 최적화 버전)
        def local_loss_function(params):
            # c_univ와 delta를 변수 목록에서 아예 배제하고, 오직 upsilon_disk만 최적화 엔진이 탐색합니다.
            # params는 최적화 엔진(minimize)으로부터 전달받는 1차원 배열(또는 스칼라) 형태입니다.
            ...
            # [2단계 이론치 완벽 동결]
            # 학계의 튜닝/보정 시비를 원천 차단하기 위해 선험적 유도 이론치로 상수를 고정(Freeze)합니다.
            c_frozen = 0.850720
            delta_frozen = 0.039513
            ...
            # 2. [물리 연산 차원 정화 패치]
            # 순정 이론 공식에 scale_factor = 0.045 좌표 변환 모듈러스를 적용하여 고속 벡터 추출
            v_tension = core.calculate_galactic_tension_velocity(r_valid, scale_factor=0.045)

            # 3. 은하 고유 평면(Intrinsic Frame)에서의 총 물리 속도 합성 및 드바이 차폐막 보정 (r_d = 3.5kpc 표준 고정치)
            ...
            # =========================================================================
            # [5단계 우주론적 정칙화 패널티 제거 및 순수 예측력 산출]
            # =========================================================================
            # c와 delta가 상수로 고정되었으므로 기존의 인위적인 패널티 수식은 완전히 무력화(소독)됩니다.
            # 오직 순수한 물리적 오차(Baryon Residuals)만을 반환합니다.
            errors = np.abs(v_predicted - v_target_valid) / v_target_valid * 100
            return np.mean(errors)

        # =========================================================================
        # 3-2. 초기 추정치 설정 (동결 검증 모드 - 1차원 최적화 빌드)
        # =========================================================================
        # [교정 완료] c_univ와 delta가 고정되었으므로, 초기값은 오직 upsilon_disk의 시작점인 [0.6] 단 1개만 전달합니다.
        initial_guess = [0.6]
        ...
        res = minimize(
            ...
            initial_guess, 
            method='Nelder-Mead', 
            bounds=open_bounds,  # 1차원 바운드 조건을 엔진에 인입
            options={
